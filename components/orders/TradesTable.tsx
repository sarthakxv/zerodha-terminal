...
export default function TradesTable({ trades, isLoading }: { trades?: KiteTrade[]; isLoading: boolean }) {
  ...
  if (!trades || trades.length === 0) {
    return (<div className="bg-bg-surface-alt border border-border"><SectionHeader title="TRADES" subtitle="0 today" /><div className="text-text-dim text-[12px] px-3 py-8 text-center">No trades today</div></div>);
  }

  return (
    <div className="bg-bg-surface-alt border border-border">
      <SectionHeader title="TRADES" subtitle={`${trades.length} today`} />
      <div className="grid grid-cols-[0.8fr_1.2fr_0.6fr_0.5fr_1fr_1fr] px-3 py-1.5 text-text-dim text-[11px] uppercase tracking-wider border-b border-[#161616]">
      ...
      {trades.map((t) => (
        <div key={t.trade_id} className="grid grid-cols-[0.8fr_1.2fr_0.6fr_0.5fr_1fr_1fr] px-3 py-1.5 text-[12px] border-b border-[#111] items-center">
          ...
          <span className="text-right text-text-dim font-mono text-[11px]">{t.order_id.slice(-8)}</span>
        </div>
      ))}
    </div>
  );
