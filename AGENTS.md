...
- `hooks/use-watchlist.ts` triggers a `react-hooks/set-state-in-effect` lint error (pre-existing). The localStorage hydration pattern is intentional to avoid SSR mismatch — see commits `0a639ac` and `0ee02c5` for context. The watchlist is capped at 50 instruments (enforced silently in `addInstrument`).
